...
class SinglePostCommentViewSet2(viewsets.ViewSet):
    serializer_class = CommentSerializer
    permission_classes = (permissions.IsAuthenticatedOrReadOnly,
                          IsOwnerOrReadOnly,)

    def create(self, request, singlepost_pk=None):
        serializer = CommentSerializer(data=request.data)
        if serializer.is_valid():
            serializer.save(author=self.request.user, parent_id=singlepost_pk)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
        return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)

    def retrieve(self, request, pk=None, singlepost_pk=None):
        queryset =  Comment.objects.filter()
        comment =  get_object_or_404(queryset, pk=pk)
        serializer = CommentSerializer(comment)
        return Response(serializer.data)
    
    def update(self, request, pk=None, singlepost_pk=None):
        comment =  Comment.objects.get(pk=pk)
        serializer = CommentSerializer(data=request.data, instance=comment)
        if serializer.is_valid():
            comment = serializer.save()
    ...
    def destroy(self, request, pk=None, singlepost_pk=None):
        try:
            comment =  Comment.objects.get(pk=pk)
        except KeyError:
            return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    @action(detail=True, methods=['get'])
    def like(self, request , pk=None, singlepost_pk=None):
        comment = Comment.objects.get(pk=pk)
        if comment.likes.filter(id=request.user.id).exists():
            comment.likes.remove(request.user)
            return Response("Like has been removed")
    ...
    @action(detail=True, methods= ["get"])
    def dislike(self, request, pk=None, singlepost_pk=None):
        comment = Comment.objects.get(pk=pk)
        if comment.dislikes.filter(id=request.user.id).exists():
            comment.dislikes.remove(request.user)
            return Response("Dislike has been removed")
...
class SinglePostReplyViewSet2(viewsets.ViewSet):
    serializer_class = ReplySerializer
    permission_classes = (permissions.IsAuthenticatedOrReadOnly,
                          IsOwnerOrReadOnly,)

    def create(self, request, singlepost_pk=None, comment_pk=None):
        serializer = ReplySerializer(data=request.data)
        if serializer.is_valid():
            serializer.save(author=self.request.user, parent_id=comment_pk)
            return Response(serializer.data, status=status.HTTP_201_CREATED)
    ...
    def retrieve(self, request, pk=None, singlepost_pk=None, comment_pk=None):
        queryset =  Reply.objects.filter(pk=pk, parent__parent=singlepost_pk, parent_id=comment_pk)
        reply =  get_object_or_404(queryset, pk=pk)
        serializer = ReplySerializer(reply)
        return Response(serializer.data)
    
    def update(self, request, pk=None, singlepost_pk=None, comment_pk=None):
        reply =  Reply.objects.get(pk=pk)
        serializer = ReplySerializer(data=request.data, instance=reply)
        if serializer.is_valid():
            reply = serializer.save()
    ...
    def destroy(self, request, pk=None, singlepost_pk=None, comment_pk=None):
        try:
            reply =  Reply.objects.get(pk=pk)
        except KeyError:
            return Response(status=status.HTTP_404_NOT_FOUND)
    ...
    @action(detail=True, methods=['get'])
    def like(self, request , pk=None, singlepost_pk=None, comment_pk=None):
        reply = Reply.objects.get(pk=pk)
        if reply.likes.filter(id=request.user.id).exists():
            reply.likes.remove(request.user)
            return Response("Like has been removed")
    ...
    @action(detail=True, methods= ["get"])
    def dislike(self, request, pk=None, singlepost_pk=None, comment_pk=None):
        reply = Reply.objects.get(pk=pk)
        if reply.dislikes.filter(id=request.user.id).exists():
            reply.dislikes.remove(request.user)
            return Response("Dislike has been removed")
